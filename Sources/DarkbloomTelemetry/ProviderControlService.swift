...
public actor ProviderControlService: ProviderControlling {
    ...
    public func performLifecycle(
        ...
        if action == .start || action == .restart {
            let savedSelectors = try await configStore.load().original.enabled
            guard !savedSelectors.isEmpty else {
            ...
            savedEnabledModels = try resolvedLocalModelIDs(
                for: savedSelectors,
                in: sources
            )
        } else {
    ...
    public func performSave(
        ...
        guard let persistedSelection = persistedSelection(
            for: draft.selection,
            in: sources
        ) else {
            throw ProviderControlError.inventoryUnavailable(Self.invalidSelectionMessage)
        }
        let draftToSave = draft.selection == persistedSelection
            ? draft
            : draft.withSelection(persistedSelection)
        let result = try await configStore.save(draftToSave)
        await onPhase?(.reconciling)
    ...
    private func refresh(
        ...
        freshResidencyRequirement: FreshResidencyRequirement?
        ...
        try requireFreshResidencyIfNeeded(
            daemonState,
            requirement: freshResidencyRequirement
        )
        appendIssue(from: daemonState, to: &sourceIssues)

        let loadedModelsState = loadedModelsFailure ?? loadedModelsFreshness(
            loadedModelsRead,
            daemon: daemonRead,
            at: capturedAt
        )
        ...
        let builtInventory = ModelInventoryBuilder.build(
            ...
            daemon: daemon,
            loadedModels: authoritativeLoadedModels
        )
        ...
        var residentModelIDs = Set(authoritativeLoadedModels)
        if let daemon {
            residentModelIDs.formUnion(daemon.warmModels)
            residentModelIDs.formUnion(daemon.slots.map(\.model))
            if !daemon.currentModel.isEmpty {
                residentModelIDs.insert(daemon.currentModel)
            }
        }
    ...
    private func loadedModelsFreshness(
        _ loadedModels: LoadedModelsState?,
        daemon: DaemonState?,
        at capturedAt: Date
    ) -> ProviderControlSourceState {
        guard let loadedModels, loadedModels.updatedAt.isFinite else {
            return .unavailable("Loaded model state timestamp is invalid")
        }
        guard capturedAt.timeIntervalSince1970.isFinite else {
            return .unavailable("Loaded model state acquisition time is invalid")
        }
        if loadedModels.updatedAt > capturedAt.timeIntervalSince1970 {
            return .stale("Loaded model state timestamp is in the future")
        }
        if let daemon,
           daemon.startedAt.isFinite,
           loadedModels.updatedAt < daemon.startedAt {
            return .stale("Loaded model state predates the current provider run")
        }
        return .fresh(evidenceAt: capturedAt)
    }
    ...
    private func requireFreshResidencyIfNeeded(
        ...
        guard let requirement, !state.isMarkedFresh else { return }
        let reason: String
        switch state {
        case .fresh:
            return
    ...
    /// ProviderConfigDocument intentionally validates raw selector membership:
    /// it has no catalog context and must remain safe for direct callers. The
    /// service has that context during its fresh save preflight, so align only
    /// preload selectors whose canonical catalog model is already enabled. This
    /// keeps unrelated raw selectors and enabled aliases intact while producing
    /// a raw TOML subset that the document validator can prove independently.
    private func persistedSelection(
        for selection: ProviderModelSelection,
        in sources: ModelSources
    ) -> ProviderModelSelection? {
        let localIDs = Set(sources.local.map(\.id))

        func resolve(_ selector: String) -> String? {
            ...
            let matches = exactMatches.isEmpty
                ...
                : exactMatches
            guard matches.count == 1, localIDs.contains(matches[0].id) else {
                return nil
            }
            return matches[0].id
        }

        var enabledIDs: [String: String] = [:]
        for selector in selection.enabled {
            guard let modelID = resolve(selector) else { return nil }
            enabledIDs[modelID] = selector
        }

        var persistedPreloads: [String] = []
        for selector in selection.preloaded {
            guard let modelID = resolve(selector),
                  let enabledSelector = enabledIDs[modelID]
            else { return nil }

            // Prefer an existing raw enabled selector. If the raw values already
            // match, this is a no-op; otherwise it removes only the alias/exact
            // mismatch that the raw document cannot reason about.
            persistedPreloads.append(enabledSelector)
        }
        return ProviderModelSelection(
            enabled: selection.enabled,
            preloaded: persistedPreloads
        )
    }
